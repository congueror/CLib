...
import com.congueror.clib.util.AlloySmelterItemHandler;
...
public class AlloySmelterTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider{
	...
	public AlloySmelterTileEntity() {
		this(TileEntityTypes.ALLOY_SMELTER_TILE_ENTITY.get());
	}
				...
				if (this.getRecipe(this.inventory.getStackInSlot(0)) != null) {
					if (this.currentSmeltTime != this.maxSmeltTime) {
						this.world.setBlockState(this.getPos(),
								this.getBlockState().with(AlloySmelterBlock.LIT, true));
						this.currentSmeltTime++;
						dirty = true;
					} else {
						this.world.setBlockState(this.getPos(),
								this.getBlockState().with(AlloySmelterBlock.LIT, false));
	...
	private ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + ConguerorLib.MOD_ID + ".alloy_smelter");
	}
	...
	private AlloySmelterRecipe getRecipe(ItemStack stack) {
		...
		Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.ALLOY_SMELTER_TYPE, this.world);
